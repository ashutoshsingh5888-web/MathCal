...
function init() {
  ...
  if (window.__MMD_AUTOSTART_TOPIC) {
    showDifficultyMenu(window.__MMD_AUTOSTART_TOPIC);
  } else {
    showHomeScreen();
  }
...
function bindInstallPrompt() {
  ...
  btn.addEventListener("click", async () => {
    trackEvent({ event: "pwa_install_click" });

    if (deferredInstallPrompt) {
      deferredInstallPrompt.prompt();
      const { outcome } = await deferredInstallPrompt.userChoice;
      deferredInstallPrompt = null;
      trackEvent({ event: "pwa_install_outcome", outcome });
      if (outcome !== "accepted") {
